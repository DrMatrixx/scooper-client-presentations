...
const PROSPECT = {
  name: "Bob",
  company: "Insurance Connection USA",
  companyDetail: "Independent insurance brokerage, 80+ carriers, 20+ years serving Texas",
  industry: "Insurance Brokerage",
  teamSize: "~8",
  painNumber: "50–100",
  painPercent: "65",
  annualCost: "150K–300K",
  competitors: {
    line: "81% of insurance CEOs rank AI as a top investment priority",
    detail: "Lemonade's AI Jim processes claims instantly. GEICO's Kate handles 100M+ interactions/year. Allstate automates 60% of inquiries. Only 6% of independent agencies have implemented AI — the window is closing fast.",
    source: "BCG 2025",
  },
  automations: [
    {
      title: "Instant Lead Response Agent",
      desc: "When a lead hits — website form, partner referral, or phone inquiry — an AI agent responds in under 60 seconds with personalized coverage info, qualifies the need, and books a consultation. 78% of insurance buyers go with whoever responds first.",
      before: { value: "Hours", sub: "to respond during business hours" },
      after: { value: "< 60 sec", sub: "24/7 instant response" },
      flow: [
        { label: "Incoming Leads", icon: "📡" },
        { label: "AI Response Agent", icon: "🧠" },
        { label: "Booked Consultation", icon: "⚡" },
      ],
      color: "#3b82f6",
      saved: "10–20 hrs/wk",
    },
    {
      title: "Smart Lead Generation & Scoring Agent",
      desc: "AI agent that identifies high-intent prospects from web traffic, partner referral data, and community signals — then scores and prioritizes them so your team calls the hottest leads first, not last.",
      before: { value: "Manual", sub: "referral tracking by hand" },
      after: { value: "Auto-scored", sub: "daily warm lead list" },
      flow: [
        { label: "Web & Partner Data", icon: "📊" },
        { label: "AI Scoring Agent", icon: "🧠" },
        { label: "Priority Lead Queue", icon: "🎯" },
      ],
      color: "#f59e0b",
      saved: "8–15 hrs/wk",
    },
    {
      title: "Medicare Enrollment Navigator Agent",
      desc: "Turns Bob's 'Navigating Medicare' expertise into a 24/7 AI assistant. Pre-screens Medicare-eligible prospects, explains plan options in plain language, answers FAQs from the book, and schedules consultations with the right agent — ready to go.",
      before: { value: "30–45 min", sub: "per intake consultation" },
      after: { value: "5 min review", sub: "AI handles pre-screening" },
      flow: [
        { label: "Medicare Inquiries", icon: "📋" },
        { label: "AI Navigator Agent", icon: "🧠" },
        { label: "Ready Consultation", icon: "✅" },
      ],
      color: "#10b981",
      saved: "12–20 hrs/wk",
    },
    {
      title: "Multi-Carrier Quote Comparison Agent",
      desc: "With 80+ carriers, finding the best rate is hours of manual work. An AI agent gathers client details, compares rates across carriers, and delivers a ranked recommendation report — cutting quote time from hours to minutes.",
      before: { value: "2–3 hours", sub: "per multi-carrier comparison" },
      after: { value: "15 min review", sub: "AI-ranked recommendations" },
      flow: [
        { label: "Client Info Input", icon: "📝" },
        { label: "AI Comparison Agent", icon: "🧠" },
        { label: "Ranked Quote Report", icon: "📄" },
      ],
      color: "#a855f7",
      saved: "10–18 hrs/wk",
    },
    {
      title: "Partner Referral & Follow-Up Agent",
      desc: "Manages your referral network — banks, mortgage companies, realtors, home builders — with automated tracking, personalized follow-ups, and nurture sequences. Zero referrals dropped, every partner feels valued.",
      before: { value: "Manual tracking", sub: "spreadsheets & memory" },
      after: { value: "Auto pipeline", sub: "zero referrals dropped" },
      flow: [
        { label: "Partner Referrals", icon: "🗂️" },
        { label: "AI Nurture Agent", icon: "🧠" },
        { label: "Conversion Report", icon: "📈" },
      ],
      color: "#ec4899",
      saved: "5–10 hrs/wk",
    },
...
const CSS = `
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  .speed-badge{animation:speedPulse 2s ease-in-out infinite}
`;
...
function SpeedCallout({ vis }) {
  return (
    <div style={{
      ...tr(vis, 0.1), borderRadius: 22, background: "#0e0e17", border: "1px solid rgba(59,130,246,0.12)",
      padding: "28px 24px", textAlign: "center", position: "relative", overflow: "hidden",
    }}>
      <Particles color="#3b82f6" on={vis} count={6} />
      <div style={{ position: "relative" }}>
        <div style={{ fontSize: 10, color: "#3b82f6", textTransform: "uppercase", letterSpacing: 2.5, fontWeight: 700, marginBottom: 16 }}>The speed-to-lead gap</div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr auto 1fr", gap: 16, alignItems: "center", marginBottom: 16 }}>
          <div>
            <div style={{ fontSize: 32, fontWeight: 900, color: "#fca5a5", lineHeight: 1 }}>
              {vis ? <Num end={78} suf="%" go={vis} /> : "0%"}
            </div>
            <div style={{ fontSize: 11, color: "#6b7280", marginTop: 4 }}>of buyers choose the<br/><strong style={{ color: "#f87171" }}>first responder</strong></div>
          </div>
          ...
          <div>
            <div style={{ fontSize: 32, fontWeight: 900, color: "#3b82f6", lineHeight: 1 }}>
              {vis ? <Num end={900} suf="%" go={vis} dur={1800} /> : "0%"}
            </div>
            <div style={{ fontSize: 11, color: "#6b7280", marginTop: 4 }}>higher conversion when<br/>responding in <strong style={{ color: "#3b82f6" }}>&lt; 5 min</strong></div>
          </div>
        </div>
        <p style={{ color: "#555", fontSize: 12, lineHeight: 1.6, maxWidth: 420, margin: "0 auto" }}>
          Every hour a lead waits, your close rate drops. Your competitors with AI respond in seconds — even at 2 AM. Here's how we'd close that gap for Insurance Connection USA.
        </p>
      </div>
    </div>
  );
...
export default function Brief() {
  ...
  const [speedRef, speedVis] = useVis(0.2);
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#08080e", fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', system-ui, sans-serif", color: "#fff", overflowX: "hidden" }}>
      ...
      <section ref={heroRef} style={{
        ...
        <div style={{ position: "absolute", top: "5%", left: "50%", transform: `translateX(-50%) translateY(${scrollY * 0.12}px)`, width: "70%", maxWidth: 450, aspectRatio: "1", borderRadius: "50%", background: "radial-gradient(circle, rgba(59,130,246,0.05) 0%, transparent 65%)", pointerEvents: "none" }} />
        ...
        <div style={{ position: "relative", maxWidth: 560 }}>
          ...
          {/* Headline — lead with speed-to-lead pain */}
          <h1 style={{ ...tr(heroVis, 0.1), fontSize: "clamp(28px, 6.5vw, 50px)", fontWeight: 900, lineHeight: 1.1, margin: "0 0 16px" }}>
            <span style={{ color: "#fff" }}>Bob, every lead that waits</span><br />
            <span style={{ background: "linear-gradient(135deg, #f43f5e, #ec4899)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>is a policy lost.</span>
          </h1>

          <p style={{ ...tr(heroVis, 0.22), color: "#555", fontSize: 14, lineHeight: 1.7, maxWidth: 440, margin: "0 auto 20px" }}>
            Your team spends <span style={{ color: "#fbbf24", fontWeight: 700 }}>{PROSPECT.painNumber} hrs/week</span> on manual work — from comparing 80+ carriers to following up on partner referrals. We found {PROSPECT.automations.length} AI automations that could reclaim that time, worth an estimated <span style={{ color: "#fbbf24", fontWeight: 700 }}>${PROSPECT.annualCost}/year</span>.
          </p>
      ...
      <section ref={speedRef} style={{ padding: "20px 16px 20px", maxWidth: 640, margin: "0 auto" }}>
        <SpeedCallout vis={speedVis} />
      </section>

      {/* ═══ 2. SOCIAL PROOF + COMPETITIVE PRESSURE ═══ */}
      <section ref={proofRef} style={{ padding: "20px 16px 20px", maxWidth: 640, margin: "0 auto" }}>
        {/* Trust bar */}
        <div style={{
        ...
        <div style={{
          ...
          <div style={{ position: "relative" }}>
            ...
            <p style={{ color: "#555", fontSize: 11, marginTop: 16, lineHeight: 1.5 }}>
              AI systems improve with data. Independent agencies that start building this advantage now will be increasingly difficult to catch.
            </p>
          </div>
        </div>
      </section>
        ...
        <div style={{
          ...
          <div style={{ position: "relative" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12 }}>
              {[
                { v: 65, s: " hrs", p: "~", sub: "freed up every week" },
                { v: 190, s: "K", p: "$", sub: "in reclaimed capacity" },
